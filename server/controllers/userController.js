...
const addEventToUser = async (req, res) => {
  ...
  try {
    const result = await User.updateOne(
      { _id: userId },
      { $addToSet: { registeredEvents: newEvent } }
    );
...
const checkInUserToEvent = async (req, res) => {
  ...
  try {
    const result = await User.updateOne(
      { _id: userId },
      { $addToSet: { checkedInEvents: newEvent } }
    );
    if (result.modifiedCount === 0) {
      res.status(404).json({
        status: "failure",
...
const checkOutUserFromEvent = async (req, res) => {
  if (req.auth.role != "admin" && req.auth.role != "user") {
    res.status(401).json({
      status: "failure",
      message: "Invalid authorization token for request.",
